...
module ODBA
  ...
  class TestPersistable < Test::Unit::TestCase
    ...
    def test_odba_id
      ODBA.cache.should_receive(:next_id).with.and_return(2)
      ODBA.marshaller.should_receive(:dump).and_return("foo")
      ODBA.storage.should_receive(:store)
      omit("No idea why this test fails")
    ...
    def test_odba_dup_not_run_before_2025
      stub = flexmock("stub")
      ...
      omit("this test was not run for a long time as it was overwritten down in this file")
    ...
    def test_extend
      ...
      str = "foo"
      str = str.extend(Persistable) # do not know how to avoid  literal string will be frozen in the future (January 2025)
      str.odba_store
      ODBA.cache.flexmock_verify
    end
