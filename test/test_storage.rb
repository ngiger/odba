...
module ODBA
  ...
  class TestStorage < Test::Unit::TestCase
    ...
    def test_delete_persistable
      dbi = flexmock("dbi")
      @storage.dbi = dbi
      expected1 = %(DELETE FROM object_connection WHERE origin_id = ?)
      dbi.should_receive(:do).once.with(expected1, 2).times(1).and_return do
        assert true
      end
      expected2 = %(DELETE FROM object_connection WHERE target_id = ?)
      dbi.should_receive(:do).once.with(expected2, 2).times(1).and_return do
        assert true
      end
      expected3 = %(DELETE FROM collection WHERE odba_id = ?)
      dbi.should_receive(:do).once.with(expected3, 2).times(1).and_return do
        assert true
      end
      expected4 = %(DELETE FROM object WHERE odba_id = ?)
    ...
    def test_create_index
      dbi = flexmock("dbi")
      @storage.dbi = dbi
      sql = %(CREATE INDEX IF NOT EXISTS origin_id_index_name
        ON index_name(origin_id))
      dbi.should_receive(:do).times(1).with(sql).and_return do
        assert true
      end
      sql = %(CREATE INDEX IF NOT EXISTS search_term_index_name
    ...
    def test_store__1
      ...
      dbi.should_receive(:do).times(1).and_return { |query, id, dump, name, prefetch, klass|
        expected = %(INSERT INTO object (odba_id, content, name, prefetchable, extent)
					VALUES (?, ?, ?, ?, ?))
    ...
    def test_store__3__name_only_set_in_db
      ...
      @storage.dbi = dbi
      dbi.should_receive(:select_one).times(1).and_return { |query, id|
        assert_equal("SELECT name FROM object WHERE odba_id = ?", query)
        assert_equal(1, id)
        {"name" => "name_in_db"}
      }
      dbi.should_receive(:do).times(1).and_return { |query, dump, name, prefetch, klass, id|
    ...
    def test_retrieve
      ...
      sql = %(SELECT target_id, COUNT(target_id) AS relevance
        ...
        GROUP BY target_id)
      dbi.should_receive(:select_all).with(sql, "foo%").and_return { assert(true) }
    ...
    def test_update_index__without_target_id
      sql = %(UPDATE index SET search_term=?
          WHERE origin_id=?)
    ...
    def test_ensure_object_connections
      ...
      sql = %(INSERT INTO object_connection (origin_id, target_id)
        VALUES (?, ?))
      sth = flexmock("sth")
  ...
  target_id INTEGER
      ...
      sql = %(CREATE INDEX IF NOT EXISTS origin_id_conditions ON conditions(origin_id);)
      @dbi.should_receive(:do).once.with(sql).and_return do
        assert true
      end
      sql = %(CREATE INDEX IF NOT EXISTS foo_conditions ON conditions(foo);)
      @dbi.should_receive(:do).once.with(sql).and_return do
        assert true
      end
      sql = %(CREATE INDEX IF NOT EXISTS bar_conditions ON conditions(bar);)
      @dbi.should_receive(:do).once.with(sql).and_return do
        assert true
      end
      sql = %(CREATE INDEX IF NOT EXISTS target_id_conditions ON conditions(target_id);)
      @dbi.should_receive(:do).once.with(sql).and_return do
        assert true
      end
    ...
    def test_collection_store
      sql = %(INSERT INTO collection (odba_id, key, value)
        VALUES (?, ?, ?))
      flexmock("StatementHandle")
    ...
    def test_index_fetch_keys
      ...
      sql = %(SELECT DISTINCT substr(search_term, 1, 2) AS key
        FROM index
        ORDER BY key)
      @dbi.should_receive(:select_all).with(sql).and_return {
        [["k1"], ["k2"], ["k3"]]
...
CREATE INDEX IF NOT EXISTS extent_index ON object(extent);)
  ...
  origin_id integer, target_id integer, PRIMARY KEY(origin_id, target_id));
CREATE INDEX IF NOT EXISTS target_id_index ON object_connection(target_id);)
  ...
  PRIMARY KEY(odba_id, key)
    ...
    def test_setup__extent
      ...
      @dbi.should_receive(:tables).and_return(tables)
      sql = "CREATE TABLE IF NOT EXISTS object_connection (origin_id integer, target_id integer, PRIMARY KEY(origin_id, target_id));"
      ...
      @dbi.should_receive(:do).once.with(sql).and_return(true)
      sql = "CREATE TABLE IF NOT EXISTS collection (odba_id integer NOT NULL, key text, value text, PRIMARY KEY(odba_id, key));"
      @dbi.should_receive(:do).once.with(sql).and_return(true)
    ...
    def test_update_condition_index__with_target_id
      flexmock("StatementHandle")
      sql = %(INSERT INTO index (origin_id, target_id, foo, bar)
VALUES (?, ?, ?, ?))
...
WHERE origin_id = ?)
    ...
    def test_ensure_target_id_index
      sql = %(CREATE INDEX IF NOT EXISTS target_id_index
        ON index(target_id))
      @dbi.should_receive(:execute).with(sql).and_return {
        raise DBI::Error
      }
